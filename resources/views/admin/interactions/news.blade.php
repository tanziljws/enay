...
@section('title', 'Interaksi Berita')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-newspaper"></i> Interaksi Berita
        </h1>
                    ...
                    <tbody>
                        @forelse($comments as $comment)
                            <tr>
                                ...
                                <td>{{ $comment->commentable->title ?? 'N/A' }}</td>
                                <td>{{ $comment->comment }}</td>
                                <td>{{ $comment->created_at->diffForHumans() }}</td>
                                <td>
                                    <form action="{{ route('admin.interactions.news.comment.delete', $comment->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
