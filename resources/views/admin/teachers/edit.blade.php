            ...
            <form action="{{ route('admin.teachers.update', $teacher) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                        ...
                        <option value="staff" {{ old('role', $teacher->role) == 'staff' ? 'selected' : '' }}>Staff</option>
                    </select>
                    @error('role')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    @if($teacher->photo)
                        <div class="mb-2">
                            ...
                            <div><img src="{{ asset('storage/' . $teacher->photo) }}" alt="{{ $teacher->name }}" 
                                     style="max-height: 120px; max-width: 120px; object-fit: cover; border-radius: 8px;"></div>
                        </div>
                    @endif
                    <input type="file" class="form-control @error('photo') is-invalid @enderror" 
                           id="photo" name="photo" accept="image/*" onchange="previewImage(this)">
                    <div class="form-text">Format: JPG, PNG, GIF. Maksimal 4MB. Kosongkan jika tidak ingin mengubah foto.</div>
